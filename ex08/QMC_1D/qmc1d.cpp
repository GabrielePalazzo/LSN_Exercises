...
{
    if (WAVEFUNCTION == UNIFORM)
        return 1.0;
    else if (WAVEFUNCTION == PSI) {
        double denominator = sqrt(2 * SIGMA * sqrt(M_PI) * (1 + exp(- MU * MU / SIGMA / SIGMA)));
        double numerator = exp(- (v - MU) * (v - MU) / 2.0 / SIGMA / SIGMA) + exp(- (v + MU) * (v + MU) / 2.0 / SIGMA / SIGMA);
...
double variationalWaveFunction_second(double v)
{
    if (WAVEFUNCTION == UNIFORM)
        ...
        double denominator = sqrt(2 * SIGMA * sqrt(M_PI) * (1 + exp(- MU * MU / SIGMA / SIGMA)));
        double numerator = -exp(- (v - MU) * (v - MU) / 2.0 / SIGMA / SIGMA) + (v - MU) * (v - MU) / SIGMA / SIGMA * exp(- (v - MU) * (v - MU) / 2.0 / SIGMA / SIGMA) - exp(- (v + MU) * (v + MU) / 2.0 / SIGMA / SIGMA) + (v + MU) * (v + MU) / SIGMA / SIGMA * exp(- (v + MU) * (v + MU) / 2.0 / SIGMA / SIGMA);
        return numerator / SIGMA / SIGMA / denominator;
    }
    else return 0;
