...
int main () {
    ...
    int nconf = 1;
    for(int istep=1; istep <= equilibriumSteps; ++istep)
    {
        Move();
        Measure();
    }
...
//update of the histogram of g(r)
            ...
            if (r_2 < half_box_2) {
                bin = igofr + sqrt(r_2)/bin_size;
                //int provv = (int)sqrt(r_2)/bin_size;
                //double rrr = (2*provv+1)*bin_size/2.0;
                r_min = (bin-igofr)*bin_size;
                r_max = (bin-igofr+1)*bin_size;
                double rrr = r_max*r_max*r_max - r_min*r_min*r_min;
                //double rrr =1;
                walker[bin] += 2.0*3/(4*pi*rrr);
            }*/
            
            if (dx*dx + dy*dy + dz*dz <  box * box / 4.0) {
                //cout << "ciao " << endl;
                bin=(int)(dr/bin_size);
                walker[igofr+bin]+=2;
            }
...
// contribution to energy and virial
    ...
    for (int k = igofr; k < igofr + nbins; ++k) {
        walker[k] *= 3.0 / (4.0 * M_PI);
        bin = k - igofr;
        double r_min = bin*bin_size;
        double r_max = (bin+1)*bin_size;
        double rrr = r_max*r_max*r_max - r_min*r_min*r_min;
        walker[k] /= rrr;
        //cout << walker[k] << endl;
    }
    ...
    ofstream Epot, Pres;
    const int wd=12;
    Epot.open("output.epot.i.0",ios::app);
    Pres.open("output.pres.i.0",ios::app);
    
    Epot << setw(wd) << walker[iv]/(double)npart + vtail << endl;
    Pres << setw(wd) << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl;
...
//g(r)
    //double normalizz = 0.0;
    
    //for (int i = igofr; i < n_props; i++) normalizz += blk_av[i]/blk_norm;
    
    for (int i = igofr; i < n_props; i++) {
        //cout << blk_av[i]/blk_norm/rho/(double)npart << endl;
        stima_gdir = blk_av[i]/blk_norm/rho/(double)npart;
        glob_av[i] += stima_gdir;
        glob_av2[i] += stima_gdir*stima_gdir;
        err_gdir = Error(glob_av[i],glob_av2[i],iblk);
        Gofr << setw(wd) << stima_gdir << endl;
        Gave << setw(wd) << i - igofr << setw(wd) << glob_av[i]/(double)iblk << setw(wd) << err_gdir << endl;
