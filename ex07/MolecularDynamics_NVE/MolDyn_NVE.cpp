    ...
    for (int i=0; i<npart-1; ++i){
        for (int j=i+1; j<npart; ++j){
            ...
            if (r_2 < half_box_2) {
                ...
                bin += igofr;
                walker[bin] += 2.0*3/(4*M_PI*rrr*rho);
            }*/
...
// Pressure
    ...
    //norm /= 4.0/3.0 * (nBins * bin_size) * (nBins * bin_size) * (nBins * bin_size);
    //cout << "---------------------" << endl;
    for (int k = igofr; k < igofr + nBins; ++k) {
        ...
        double rrr = r_max*r_max*r_max - r_min*r_min*r_min;
        walker[k] /= rrr;
        //cout << walker[k] / rho/(double)npart<< endl;
    }
    /*
...
void Averages(int iblk) //Print results for current block
{
    ...
    for (int i = igofr; i < n_props; i++) {
        ...
        double observable = blk_av[i]/blk_norm/rho/(double)npart;
        glob_av[i] += observable;
        glob_av2[i] += observable*observable;
        double err_gdir = Statistical_uncertainty(glob_av[i]/(double)iblk, glob_av2[i]/(double)iblk, iblk);
        //if (i == 16+igofr) cout << i << " " << glob_av[i]/(double)iblk << " " << observable << endl;
        Gofr << setw(wd) << stima_gdir << endl;
        Gave << setw(wd) << i - igofr << setw(wd) << glob_av[i]/(double)iblk << setw(wd) << err_gdir << endl;
        //if (iblk > 99) cout << setw(wd) << i - igofr << setw(wd) << glob_av[i]/(double)iblk << setw(wd) << err_gdir << endl;
    }
    Gofr.close();
    Gave.close();
