...
//    Pick(previousPopulation, fit);
    ...
    if (CHINESE) {
        for (int i = 0; i < numOfPopulations; i++) {
            ...
            if (new_fit < avg_fit) {
                if (probability < maxMutationRate) Mutate(cityArray, newPopulation[i], rnd);
            }
            else {
                if (probability < 1*(maxMutationRate - minMutationRate)*(new_fit-avg_fit)/(max_fit-avg_fit)) Mutate(cityArray, newPopulation[i], rnd);
            }
        }
    }
    else {
        for (int i = 0; i < numOfPopulations; i++) {
            //double probability = Rand_uniform();
            double probability = rnd->Rannyu();
            if (probability < mutationRate) Mutate(cityArray, newPopulation[i], rnd);
        }
    }
...
void Mutate (City* cityArray, int* pickedOrder, Random* rnd) {
